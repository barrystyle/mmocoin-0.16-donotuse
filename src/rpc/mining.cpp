...
UniValue GetNetworkHashPS(int lookup, int height) {
    ...
    //ppcTODO - redo this to fit mmocoin
...
// mmocoin: get network Gh/s estimate
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated mmocoin to.\n"
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when mmocoind is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
UniValue submitblock(const JSONRPCRequest& request)
{
    ...
    // mmocoin: check block before attempting to sign it
    CValidationState state;
